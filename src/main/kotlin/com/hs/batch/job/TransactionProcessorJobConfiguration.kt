...
import com.hs.batch.entity.ClassInformation
...
class TransactionProcessorJobConfiguration(
    ...
    companion object {
        private const val JOB_NAME = "TransactionProcessorBatch"
    ...
    fun step(): Step {
        return stepBuilderFactory.get(BEAN_PREFIX + "step")
            .chunk<Teacher, ClassInformation>(chunkSize)
            .reader(reader())
            .processor(processor())
            .writer(writer())
            .build()
    ...
    fun processor(): ItemProcessor<Teacher, ClassInformation> {
        return ItemProcessor { teacher ->
            ClassInformation(teacherName = teacher.name, studentSize = teacher.students.size)
        }
    }
